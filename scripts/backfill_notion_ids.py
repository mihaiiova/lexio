...
continue to resolve without a migration.
...
def main():
    ...
    grammar = [
        insert_after(ex, "id", "notionId", ex["pairId"]) for ex in grammar
    ]
    ...
    vocab = [insert_after(ex, "id", "notionId", ex["word"]) for ex in vocab]
    ...
    idioms = [
        insert_after(ex, "id", "notionId", ex["expression"]) for ex in idioms
    ]
    ...
    for text in spot:
        text["mistakes"] = [
            insert_after(m, "wordIndex", "notionId", spot_notion_id(m))
    ...
    common = [
        {"notionId": f"gp{i}", **pair} for i, pair in enumerate(common)
    ]
    ...
    hyphenation = [
        insert_after(p, "id", "notionId", f"hyphenated_{p['id']}")
